alter table saved_words
  add column if not exists next_practice_at timestamptz not null default (now() + interval '24 hours'),
...
update saved_words
set next_practice_at = coalesce(next_practice_at, created_at + interval '24 hours')
